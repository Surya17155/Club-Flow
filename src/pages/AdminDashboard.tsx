      ...
      <main className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-3 h-full">
          <div className="glass-card p-6 h-full flex flex-col items-center text-center">
            ...
            {isPersonal ? (
              <span
                className="mt-1.5 mb-1 inline-block px-3 py-0.5 rounded-full text-xs font-semibold"
                style={{ backgroundColor: "hsl(200 60% 95%)", color: "hsl(200 60% 40%)" }}
              >
                Student
              </span>
            ) : (
              <>
                <span
                  className="mt-1.5 mb-1 inline-block px-3 py-0.5 rounded-full text-xs font-semibold"
                  style={{ backgroundColor: "hsl(270 60% 95%)", color: "hsl(270 60% 45%)" }}
                >
                  {roleLabel}
                </span>
            ...
            <div className="mt-auto flex gap-4 justify-center mt-5">
              {profile?.social_linkedin && (
                <a
                  href={
                    profile.social_linkedin.startsWith("http")
                      ? profile.social_linkedin
                    ...
                    clubs.map((club) => (
                      <div key={club.club_id} className="flex items-center gap-4 group cursor-pointer">
                        ...
                        <div>
                          ...
                          <span className="text-xs text-muted-foreground">{roleLabelMap[club.role] ?? club.role}</span>
                        </div>
                      </div>
                    ))
                  ) : (
