        ...
        <nav ref={navRef} onScroll={handleNavScroll} className="flex-1 flex flex-col gap-1 px-3 mt-2 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
            ...
            <>
              ...
              <AnimatePresence initial={false}>
                {isSuperAdminMode && (
                  ...
                  >
                    <div className="ml-3 pl-2 mt-1 space-y-1" style={{ borderLeft: `2px solid ${isNeo ? '#333' : 'rgba(255,255,255,0.12)'}` }}>
          ...
          {isSuperAdminEmail && collapsed && (
            <>
              ...
              {isSuperAdminMode && superAdminSubItems.map((sub) => {
                const subActive = sub.activeUrl ? isActive(sub.activeUrl) : false;
                return (
                  <div key={sub.title} className="group relative">
                    <MagnifiedIcon
                      item={{ title: sub.title, icon: sub.icon, url: '' }}
                      active={subActive}
                      mouseY={mouseY}
                      onClick={sub.action}
                      isNeo={isNeo}
